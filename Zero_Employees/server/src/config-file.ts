...
export function readConfigFile(): PaperclipConfig | null {
  ...
  try {
    ...
    throw new Error(`Invalid Zero config at ${configPath}: failed to read or parse JSON: ${reason}`);
  }
  ...
  try {
    ...
    if (error instanceof ZodError) {
      throw new Error(`Invalid Zero config at ${configPath}: ${formatConfigValidationError(error)}`);
    }
