...
export function connectionIntentOAuthOutcomeHtml(input: {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Connection authorization</title></head><body><p>Returning to Zero…</p><script>const message=${message};const targetOrigin=${targetOrigin}||window.location.origin;if(window.opener&&window.opener!==window){window.opener.postMessage(message,targetOrigin);window.close();}else{window.location.replace(${fallback});}</script></body></html>`;
}
...
export function toolAccessRoutes(
  ...
  function oauthRedirectUri(req: Request) {
    ...
    if (!baseUrl) {
      throw unprocessable(
        "This Zero needs a browser-reachable HTTPS address (or loopback HTTP) before browser sign-in can start.",
...
/**
  ...
  router.post("/agents/me/connections/:connectionId/token", validate(connectionTokenRequestSchema), async (req, res) => {
    ...
    const headerRunId = req.get("X-Zero-Run-Id")?.trim();
      ...
      credentialSources: {
        vercelConnect: {
          ...
          reason: vercelConnect.enabled
            ...
            : "Vercel Connect setup is disabled on this Zero instance.",
        },
      },
  ...
  /**
   * Zero's Client ID Metadata Document (PAP-17087).
   *
   * The document's own URL is the `client_id` Zero presents to an
   ...
   * Zero session. It contains only this deployment's callback and the
   * grant/response/auth methods Zero uses: no company, connection or secret
   * data of any kind.
   */
  ...
  router.get("/tools/oauth/callback", async (req, res) => {
    ...
    // provider's body: they are provider-authored prose, and Zero maps the
    ...
    try {
      ...
      await logActivity(db, {
        ...
        details: {
          ...
          // HttpError messages are Zero-authored. Provider-authored
