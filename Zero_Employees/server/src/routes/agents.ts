...
// the right scope because a Zero instance serves its API from one
...
export function agentRoutes(
   ...
   *   adapter probes the Zero host (legacy behavior).
  ...
  async function assertSelectableAdapterType(type: string | null | undefined): Promise<string> {
    ...
    if (adapterType === "paperclip_runner") {
      ...
      if (experimental.enableNativeRunner !== true) {
        throw unprocessable(
          "Zero Runner is experimental and disabled on this instance.",
  ...
  function resolvePaperclipRunnerAdapterTransition(input: {
    ...
    if (!["claude_local", "codex_local", "opencode_local"].includes(input.previousAdapterType)
      ...
      throw unprocessable("Select a Zero Runner provider before converting this agent.");
    }
  ...
  // the Zero operational skill as a runtime invariant.
  ...
  router.get("/companies/:companyId/adapters/:type/models", async (req, res) => {
    ...
    if (type === "paperclip_runner" && provider && !isPaperclipRunnerProvider(provider)) {
      throw unprocessable("Unknown Zero Runner provider");
    }
  ...
  // the Zero host, so it is authoritative for the null-environment and
   ...
   * Zero over plain HTTP on a home server or a Tailscale tailnet. So the
