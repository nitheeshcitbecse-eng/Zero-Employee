...
describe("Chat SDK published adapter integration", () => {
  ...
  it("acknowledges a signed Slack slash command without a blocking profile lookup", async () => {
    ...
    try {
      ...
      const usersInfo = vi.fn(
        async () =>
          await new Promise<never>(() => {
            ...
            // acknowledgement open until Zero's webhook deadline.
          }),
      );
      ...
      await expect((response as Response).json()).resolves.toEqual({
        ...
        text: "Zero received this command.",
      });
  ...
  it.each([
    ...
    "stops Telegram attachment downloads at Zero's byte cap with $label",
    async ({ contentLength }) => {
    ...
    async (scope) => {
      ...
      try {
        ...
        const edited = await adapter.editMessage(threadId, "owned-progress", {
          markdown:
            "This chat conversation is closed. The Zero task remains available.",
        });
        ...
        expect(body).toMatchObject({
          type: "message",
          textFormat: "markdown",
          text: "This chat conversation is closed. The Zero task remains available.",
        });
  ...
  it("persists Teams metadata only after Zero admits the authenticated activity", async () => {
    const state = memoryPersistence();
