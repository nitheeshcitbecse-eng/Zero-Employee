...
/**
 * Naming convention for npm-published Zero plugins.
 * Packages matching this pattern are considered Zero plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
...
export interface PluginLoader {
  ...
  /**
   * Discover Zero plugins installed as npm packages in the current
   ...
   *
   * Returns null if the package is not a Zero plugin.
   * Throws if the package is a Zero plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
...
/**
 * Check whether a package name matches the Zero plugin naming convention.
...
export function pluginLoader(
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      const manualBuildHint = localPath
        ...
        : "";
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Zero plugin (no manifest found).${manualBuildHint}`,
      );
    }
  ...
  async function refreshPluginManifestFromPackage(
    ...
    if (!manifest) {
      throw new Error(`Plugin package ${plugin.packageName} no longer exposes a Zero manifest`);
    }
   ...
   * if the package is not a Zero plugin.
   */
