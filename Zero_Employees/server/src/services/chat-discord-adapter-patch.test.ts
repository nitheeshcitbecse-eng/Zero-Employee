...
describe("Zero Discord adapter patch", () => {
  afterEach(() => {
    vi.useRealTimers();
    vi.unstubAllGlobals();
  });
  ...
  it("never logs inbound message content before Zero admission", async () => {
    const { adapter, chat, client, handlers, logger } = harness();
    await adapter.initialize(chat as never);
    (
      adapter as unknown as {
  ...
  it("checks Zero admission before creating a root provider thread", async () => {
  ...
  it("does not acknowledge an action that Zero durably rejects", async () => {
    const { adapter, chat, client, handlers, logger } = harness();
    ...
    expect(interaction.reply).toHaveBeenCalledWith({
      content:
        "This action is no longer available. Open the linked Zero task or ask an operator to link this account.",
      flags: 64,
    });
    expect(logger.info).toHaveBeenCalledWith(
      "Discord Gateway action was not acknowledged after Zero rejected it",
      expect.objectContaining({
        event: "interaction",
        messageId: "message-2",
      }),
  ...
  it("acknowledges a duplicate that Zero reports as already durable", async () => {
    const { adapter, chat, client, handlers } = harness();
    await adapter.initialize(chat as never);
    (
      adapter as unknown as {
