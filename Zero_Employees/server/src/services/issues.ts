...
export function issueService(db: Db) {
  ...
  const service = {
    ...
    create: async (
      ...
      const persist = async (tx: DbTransaction) => {
        ...
        if (workspaceInheritanceIssueId) {
          ...
          // cross-project child (for example, a Zero ID issue created from
          // a Zero App parent) must fall through to its own project's
    ...
    addComment: async function addComment(
      ...
      // Zero even while a bound conversation is active.
      if (authorType === "agent" && isExplicitExternalAgentComment(metadata)) {
        // An external-chat run may perform ordinary Zero lifecycle or
