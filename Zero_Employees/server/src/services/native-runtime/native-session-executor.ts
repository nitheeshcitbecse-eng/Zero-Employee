  ...
  try {
    const confirmation = await issueThreadInteractionService(input.db).create(
      ...
      },
      {
        ...
        summary: "Review the synchronized Zero plan.",
        continuationPolicy: "wake_assignee",
        payload: {
          version: 1,
...
export async function executePaperclipNativeSession(input: {
  ...
  useRunnerd?: boolean;
  /** Zero adapter identity used to scope the durable goal projection. */
  adapterType?: string;
...
export function createRemoteRunnerProcessLauncher(input: {
  ...
  return (spec) => {
    ...
    const completion = (async () => {
      ...
      // that RPC even when Zero requests a longer timeout. Detach runnerd
