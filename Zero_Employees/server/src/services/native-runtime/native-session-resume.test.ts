...
describe("rebindNativeSessionCheckpoint", () => {
  ...
  it.each([
    ...
    "refreshes retained $contract without changing the Zero task or prior history",
    ({ retainedFingerprint }) => {
        ...
        buildExecution: (options) => {
          ...
          return {
            ...
            task: {
              ...
              prompt: options.resumedSession
                ? "Zero Resume Delta"
                : "Full context for the same Zero task",
            },
          };
        },
      });
      ...
      // No deletion/reset of the existing provider checkpoint or Zero history.
...
describe("buildNativeExecutionInput wake projection", () => {
  ...
  it.each([false, true])("projects wake context with the appropriate execution contract (conversation=%s)", (conversationMode) => {
    ...
    expect(input.task.prompt).toContain("## Zero Resume Delta");
