...
const NATIVE_QUESTION_GUIDANCE = [
  ...
  "Zero renders the supported question controls and authenticates the answer. Never fabricate answer URLs, query-string choice links, callback tokens, or fake Markdown buttons. Do not manually post a duplicate question card or use call_api as a substitute.",
...
const NATIVE_GITHUB_ATTACHMENT_RECOVERY_GUIDANCE = [
  ...
  "Zero owns recovery navigation for unavailable GitHub attachments. It may append an authenticated task link after an accepted response, only when the current source remains authorized and a safe configured Board URL is available. The model does not select or authorize that link.",
  "A task URL missing from your prompt or tool results is not evidence that no task link can be provided; do not claim that a link is unavailable merely because you cannot see its URL. Do not invent a URL or promise that a link will appear. Briefly explain the unavailable input and ask the user to attach it directly to this Zero task or paste the needed text. Never infer the file's contents or substitute an older file.",
].join("\n");
...
export function buildNativeExecutionInput(input: {
  ...
  taskPrompt: string;
  /**
   * The already-sanitized Zero wake envelope for this run. Native drivers
