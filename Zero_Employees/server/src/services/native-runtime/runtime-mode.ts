...
export function resolveNativeRuntimeMode(input: {
  ...
  if (
    ...
    throw ineligible(
      ...
      "Zero Runner requires an active agent.",
    );
  }
  ...
  if (!input.issue || !allowedWorkModes.includes(input.issue.workMode)) {
    throw ineligible(
      ...
      "Zero Runner requires a standard, planning, or ask task.",
    );
  }
    ...
    if (input.persisted.runtimeMode === "native") {
      if (input.agent.adapterType !== "paperclip_runner") {
        throw ineligible(
          ...
          "A persisted native run must remain bound to the Zero Runner adapter.",
        );
      }
      if (
        ...
        throw ineligible(
          "paperclip_runner_agent_ineligible",
          "A persisted Zero Runner run cannot recover through a non-invokable agent.",
        );
      }
      ...
      if (backend === null) {
        throw ineligible(
          ...
          `Persisted Zero Runner driver is unsupported: ${driverKind}`,
        );
      }
      return {
        kind: "native",
