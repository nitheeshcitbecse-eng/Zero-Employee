  ...
  if (
    ...
    if (provider === "codex") {
      throw new PaperclipRunnerProviderProfileError(
        ...
        "Zero Runner currently supports Codex only with codexPermissionMode set to never. Update the agent configuration before starting a new native run.",
      );
    }
    throw new PaperclipRunnerProviderProfileError(
...
/**
 * Resolve the immutable provider identity used for a fresh Zero Runner
...
export function resolvePaperclipRunnerProviderProfile(
  ...
  if (!isPaperclipRunnerProvider(candidate)) {
    throw new PaperclipRunnerProviderProfileError(
      ...
      "Zero Runner provider must be Codex, OpenCode, Claude Managed, AWS AgentCore, or ACPX.",
    );
  }
  ...
  if (candidate === "opencode") {
    if (!model || !model.includes("/") || model.endsWith("/")) {
      throw new PaperclipRunnerProviderProfileError(
        ...
        "Zero Runner OpenCode requires model in provider/model form.",
      );
    }
    return {
      provider: "opencode",
  ...
  if (candidate === "claude_managed") {
    ...
    if (!managedProfileId) {
      throw new PaperclipRunnerProviderProfileError(
        ...
        "Zero Runner Claude Managed requires a company managed-agent profile.",
      );
    }
    if (config.managedAgentsRetentionAcknowledged !== true) {
      throw new PaperclipRunnerProviderProfileError(
        "paperclip_runner_claude_managed_retention_required",
        "Zero Runner Claude Managed requires acknowledgement of stateful beta retention.",
      );
    }
    ...
    return {
      ...
      maxSessionListCostUsd: positiveNumberOrNull(
        ...
        "Zero Runner Claude Managed requires a positive session spend ceiling when overridden.",
      ),
    };
  }
  ...
  if (candidate === "aws_agentcore") {
    ...
    if (!agentCoreProfileId) {
      throw new PaperclipRunnerProviderProfileError(
        ...
        "Zero Runner AWS AgentCore requires a company remote-agent profile.",
      );
    }
    if (config.agentCoreRetentionAcknowledged !== true) {
      throw new PaperclipRunnerProviderProfileError(
        "paperclip_runner_aws_agentcore_retention_required",
        "Zero Runner AWS AgentCore requires acknowledgement of 90-day Memory retention.",
      );
    }
    ...
    return {
      ...
      maxEstimatedSessionCostUsd: positiveNumberOrNull(
        ...
        "Zero Runner AWS AgentCore requires a positive estimated session spend ceiling when overridden.",
      ),
    };
  }
  ...
  if (acpxAgent !== "claude" && acpxAgent !== "codex") {
    throw new PaperclipRunnerProviderProfileError(
      ...
      "Zero Runner ACPX requires the qualified Claude or Codex agent profile; Pi is not available.",
    );
  }
  ...
  if (acpxAgent === "codex" && model !== qualifiedModel) {
    throw new PaperclipRunnerProviderProfileError(
      ...
      `Zero Runner ACPX ${acpxAgent} requires exact model ${qualifiedModel}.`,
    );
  }
  return {
    provider: "acpx",
...
export function resolvePaperclipRunnerNativeProviderInput(input: {
  ...
  if (profile.backend !== input.backend) {
    throw new PaperclipRunnerProviderProfileError(
      ...
      "Zero Runner provider changed after this run selected its native backend.",
    );
  }
