...
describeEmbeddedPostgres("current external-chat wake comment reader", () => {
  ...
  it("surfaces closed attachment omissions and fences their exact snapshot", async () => {
    ...
    expect(page.comments[0]?.attachmentImportNotice).toBe(
      "Zero could not import every attachment from this exact external message: 2 attachments were omitted (unsupported type: 1, processing failed: 1). Treat omitted attachments as unavailable; do not infer their contents or substitute an older workspace file.",
    );
