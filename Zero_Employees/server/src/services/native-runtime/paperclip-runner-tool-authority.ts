...
export class PaperclipRunnerToolAuthority {
  ...
  definitions(): Array<Record<string, unknown>> {
    ...
    const definitions: Array<Record<string, unknown>> =
      CAPABILITY_SEMANTIC_TOOL_CATALOG.filter(
        ...
        description:
          descriptor.operationId === "register_deliverable"
            ? "Prepare one verified workspace file for Zero's final task or external-chat response. This records the attachment, work product, and explicit same-run selection; it does not confirm provider delivery."
            : descriptor.operationId === "request_human_input"
              ? "Create a typed, durable human-input interaction on the current Zero task bound to this run. For structured questions and choices, use interactionKind 'questions' with payload.questions as described by the payload schema. Zero renders the interaction in its UI and, for connected chats, uses supported provider question controls or a safe fallback. Normal task permissions and review gates still apply."
  ...
  async #callApi(callId: string, value: unknown): Promise<unknown> {
    ...
    const apiUrl = this.binding.apiUrl ?? process.env.PAPERCLIP_API_URL;
    if (!apiUrl) throw new Error("Zero API origin is unavailable");
    ...
    if (!token) throw new Error("Zero run authentication is unavailable");
    const execute = async () => {
