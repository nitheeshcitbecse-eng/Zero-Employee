...
describe("native execution input external-chat framing", () => {
  ...
  it.each([false, true])(
    ...
    (resumedSession) => {
      ...
      expect(input.task.prompt).toContain(
        "Zero owns recovery navigation for unavailable GitHub attachments",
      );
      expect(input.task.prompt).toContain(
      ...
      expect(input.task.prompt).toContain(
        "Briefly explain the unavailable input and ask the user to attach it directly to this Zero task or paste the needed text",
      );
      expect(input.task.prompt).toContain(
