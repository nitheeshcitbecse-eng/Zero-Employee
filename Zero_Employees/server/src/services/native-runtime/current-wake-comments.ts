...
function attachmentImportNotice(
  ...
  const reasons = entries
    ...
    .join(", ");
  return `Zero could not import every attachment from this exact external message: ${omitted} attachment${omitted === 1 ? " was" : "s were"} omitted (${reasons}). Treat omitted attachments as unavailable; do not infer their contents or substitute an older workspace file.`;
}
...
async function currentWakeCommentsSnapshot(
  ...
  for (const attachment of attachments) {
    ...
    current.push({
      ...
      // This closed reader intentionally has no general Zero API key.
