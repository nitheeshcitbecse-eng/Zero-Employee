 ...
 * connection, or tool side effect stay in Zero, where the complete
...
function textForQuestionInteraction(
  ...
  lines.push(
    taskUrl
      ? `Open the task in Zero to respond: ${taskUrl}`
      : "Open the task in Zero to respond.",
  );
  return lines.join("\n");
}
...
function genericInteractionText(taskUrl: string | null): string {
  return [
    "This task needs an authorized response in Zero.",
    taskUrl
      ? `Open the task in Zero to respond: ${taskUrl}`
      : "Open the task in Zero to respond.",
  ]
...
export async function enqueueIssueInteractionChatPublications(
  ...
  // terminal semantics that are authoritative in Zero; projecting a
 ...
 * providers with buttons, so an "Open in Zero" prompt never remains
 ...
 * Zero UI wins, all still-issued callback tokens expire in this same
