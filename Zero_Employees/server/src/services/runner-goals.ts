...
function capabilityForAgent(agent: AgentBinding): RunnerGoalCapability {
  ...
  if (agent.adapterType === "paperclip_runner") {
    ...
    const reason = provider === "opencode"
      ...
      : provider === "acpx"
        ...
        : "This Zero runner provider does not expose a durable session goal lifecycle.";
    return {
  ...
  const reason = agent.adapterType === "opencode_local"
    ? "Unsupported by OpenCode."
    : directAcp
      ? "This direct ACP adapter has no live session goal controller. Use Zero Runner with a supported provider."
