 ...
 * Zero went on reporting the leased lane `stopped` and its exposure
 * `removed`.
 *
...
export type ExposureReservationSource =
  ...
  | "in_memory_runtime"
  /** The broker reports a live Zero-owned Serve mapping on the port. */
  | "broker_mapping"
...
/** One Zero-owned listener as reported by the broker's `list()`. */
...
export interface ExposureReservationDrift {
  /** The row Zero believes is stopped/removed. */
  runtimeServiceId: string;
  owner: ExposureOwnerIdentity;
  port: number;
