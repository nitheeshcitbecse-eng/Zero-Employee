...
function serviceCommand() {
  // Answers `/api/health` the way a real Zero dev runtime does: managed
...
/**
 * The pre-feature Zero App project template, verbatim: a hard-coded HTTP
...
function startInput(options?: {
  ...
  return {
    ...
    actor: { id: null, name: "Zero", companyId: "aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee" },
    issue: null,
    workspace: {
      baseCwd: process.cwd(),
...
describe("loopback bind is forced on the guest, not merely requested (PAP-17256)", () => {
  ...
  it("leaves a non-Zero service's --bind argument alone", async () => {
  ...
  it("does not rewrite a Zero dev command when the service is not exposed", async () => {
    const { broker, calls } = createBroker();
    installDeps({ broker });
