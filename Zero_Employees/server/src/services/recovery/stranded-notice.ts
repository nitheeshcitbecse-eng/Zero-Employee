...
export const DEFAULT_STRANDED_RECOVERY_NOTICE_BODY =
  "Zero could not restore a live execution path for this issue automatically. " +
...
export function buildImmediateExecutionPathRecoveryNoticeSeed(input: {
  ...
  const retryDescription = input.status === "todo"
    ? "Zero automatically retried dispatch for this assigned `todo` issue during terminal run recovery"
    : "Zero automatically retried continuation for this assigned `in_progress` issue during terminal run recovery";
  return {
    body:
...
export function buildWorkspaceValidationRecoveryNoticeSeed(): StrandedRecoveryNoticeSeed {
  return {
    body:
      "Zero stopped before launching the local adapter because the issue workspace failed validation. " +
...
export function buildConfigurationIncompleteRecoveryNoticeSeed(
  ...
  if (readNonEmptyStringField(configurationIncomplete, "reason") === SANDBOX_PROVIDER_PLUGIN_NOT_READY_REASON) {
    ...
    return {
      body:
        `Zero stopped before dispatching the adapter because the sandbox provider plugin \`${pluginKey}\` ` +
  ...
  return {
    body:
      "Zero stopped before dispatching the adapter because required secret/env bindings are missing. " +
...
export function buildExecutionReviewParticipantRecoveryNoticeSeed(): StrandedRecoveryNoticeSeed {
  return {
    body:
      "Zero retried the pending execution-review participant once, but the review stage still has no " +
...
export function buildExecutionReviewParticipantUnavailableNoticeSeed(): StrandedRecoveryNoticeSeed {
  return {
    body:
      "Zero cannot continue the pending execution-review participant because the participant is not " +
