...
const CHAT_QUESTION_FORM_DENIAL_MESSAGE =
  "This form is no longer authorized. Close it and open the linked Zero task.";

/**
 * Provider callbacks must be acknowledged after Zero has durably denied a
...
function formKind(
  ...
  if (source) {
    if (source.answerMode === "text") {
      ...
      // Keep those uncommon forms in Zero, whose canonical validator owns
...
/**
 * Converts opaque submitted values to canonical Zero answers. Provider
