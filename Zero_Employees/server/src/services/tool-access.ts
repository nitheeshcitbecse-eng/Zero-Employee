 ...
 * `error_description`, `error_uri`, and the response body. Zero surfaces
 ...
 * key here") into Zero's own voice.
 *
 * The rule is therefore: the operator only ever reads text Zero authored.
...
/**
 * Stable, Zero-authored operator copy for each allowlisted provider error.
 * Deliberately keyed on the code alone: the calling context is already carried
 * by the Zero `code` in `details`, so one table serves the callback,
...
const OAUTH_PROVIDER_ERROR_MESSAGES: Record<string, string> = {
  ...
  invalid_client: "The authorization server rejected Zero's OAuth client.",
  invalid_client_metadata:
    "The authorization server rejected Zero's client registration details.",
  invalid_grant:
    "The authorization server rejected the authorization code or refresh token.",
  invalid_redirect_uri:
    "The authorization server rejected Zero's callback URL.",
  invalid_request:
  ...
  unapproved_software_statement:
    "The authorization server rejected Zero's client registration details.",
  unauthorized_client:
    "The authorization server refused to authorize Zero's OAuth client.",
  unsupported_grant_type:
    "The authorization server does not support the grant Zero uses.",
  unsupported_response_type:
    "The authorization server does not support the sign-in flow Zero uses.",
};
...
/** Zero's own message for a provider failure, never the provider's. */
 ...
 * own URL is the `client_id` Zero presents, so this path is a stable part of
...
/**
 * Resolve the URL Zero would use as a CIMD client id, but only when its
 * hostname is not known to resolve into a private network.
 *
 * An authorization server fetches this URL from outside Zero's network and
...
/**
 * Zero's client metadata for CIMD (RFC 7591 metadata, served rather than
...
export function oauthClientIdMetadataDocument(input: {
  ...
  return {
    ...
    client_name: `Zero (${new URL(input.redirectUri).host})`,
...
type OAuthProviderEndpoints = {
   ...
   * restrict the access token to this server rather than to everything Zero
   * has ever connected.
   */
...
> = {
  "paperclip.echo-calculator-time": {
    name: "Zero Echo / Calculator / Time fixture",
    tools: [
      {
        name: "echo",
  ...
  "paperclip.synthetic-todo-kv": {
    name: "Zero Synthetic Todo / KV fixture",
    tools: [
      {
        name: "list_items",
...
const TOOL_EXAMPLES: ToolExampleDefinition[] = [
  {
    ...
    applicationName: "Zero example: Safe read-only Todo / KV",
    applicationDescription:
      "Deterministic MCP fixture for first-run tool governance checks.",
    connectionName: "Zero example: Safe read-only Todo / KV",
    ...
    profileDescription:
      "Allows only the read-only tools from the Zero Todo / KV example fixture.",
  },
];
...
export async function loadGitHubGrantMetadata(
  ...
  if (installationIds.length === 0 || repositoryCount === 0) {
    ...
    throw unprocessable(
      "GitHub access is required. Install Zero and grant at least one repository before refreshing access.",
      {
 ...
 * is Zero's own decision about an unsafe address, so it must keep its code
...
export function toolAccessService(
   ...
   *
   * Every OAuth endpoint Zero acts on is attacker-influenced: discovered
   ...
   * Zero's own origin is exempt from the transport rule because a
  ...
  /**
   * Origins that are Zero itself: this deployment's configured public URL,
  ...
  /** Throws unless `value` is an endpoint Zero may use (and navigate to). */
  ...
  function buildRuntimeAlerts(input: {
    ...
    return [
      ...
      runtimeAlert({
        ...
        firstResponderAction:
          "Check upstream MCP health, Zero runtime capacity, and recent gateway audit failures before retrying workloads.",
        runbookSection,
      }),
      runtimeAlert({
   ...
   *    Zero secret rather than a per-user credential.
  ...
  async function oauthProviderEndpoints(
    ...
    // A gallery default is Zero's own data, but it is still a URL that ends
  ...
  async function registerOAuthClient(input: {
    ...
    const requestedMetadata = {
      ...
      // RFC 7591: Zero's callback is a server-side HTTPS endpoint, so this
    ...
    // client. Zero still uses only the exact redirect, grant and response
    ...
    // metadata inherits the values Zero requested. Additional provider-owned
    // values do not widen Zero's behavior because they are never persisted as
   ...
   * `client_id` *is* the https URL of Zero's published client metadata, so
   ...
   * as `manual`, which is the conservative answer: Zero will not silently
  ...
  function oauthClientBindingMatches(
    ...
    // durable proof that Zero minted that client. Force a fresh registration
    ...
    // Zero's callback, so it has no recorded callback until first use.
  ...
  /**
   * May Zero mint client material for this connection without an operator
  ...
  async function ensureOAuthClient(input: {
    ...
    if (
      ...
      // Zero cannot re-register on the operator's behalf: the credentials
  ...
  async function exchangeOAuthToken(input: {
    ...
    // Zero without re-checking the transport it would leave over.
    const tokenUrl = assertOAuthEndpointUrl("token", input.tokenUrl, {
      // Zero's own callback origin, so a first-party token endpoint keeps
  ...
  async function connectGalleryApp(
    ...
    if (credentialSource === "vercel_connect") {
      ...
      if (!integration.enabled || !integration.configured || !vercelConnect) {
        throw unprocessable(
          "Vercel Connect setup is not available on this Zero instance",
          {
            code: "vercel_connect_unavailable",
          },
        );
      ...
      if (expectedPrincipalMode === "app") {
        ...
        if (connectorInUse) {
          throw conflict(
            "App-subject Vercel connectors are dedicated to one Zero connection. Create or attach a separate connector in Vercel.",
            {
    ...
    // the client id now; the secret becomes an encrypted Zero secret below.
    ...
    try {
      ...
      if (existingApplication) {
        ...
        if (!applicationRow) {
          throw conflict(
            "Zero could not allocate a unique connection name",
            {
        ...
        if (
          ...
          const endpoints = await discoverOAuthEndpoints(oauthConnection).catch(
            (discoveryError: unknown) => {
              // "This server advertised an address Zero refuses to open" is a
  ...
  async function reconnectGalleryApp(
    ...
    if (connection.credentialSource === "vercel_connect") {
      throw conflict(
        "Manage this connector in Vercel Connect, then run a Zero health check to verify it.",
        {
  ...
  async function startOAuth(
    ...
    if (
      ...
      if (!cloudConnector) {
        throw unprocessable(
          `${providerName} connections through Zero are not available on this instance yet`,
          {
    ...
    const authorizationUrl = new URL(
      assertOAuthEndpointUrl("authorization", endpoints.authorizationUrl, {
        // Zero's own callback origin: a first-party authorization endpoint is
    ...
    // MCP URLs retain discovery-first behavior because Zero has no manifest
    ...
    const nextConfig = {
      ...
      oauth: {
        ...
        // universe and must never silently become Zero's requested set.
    ...
    await db
      ...
      .set({
        ...
        result: {
          version: 1,
          outcome: "rejected",
          // Zero's own words: the provider's explanation is untrusted and
  ...
  async function completeOAuthCallback(input: {
    ...
    // acted on once the callback is bound to a state Zero issued and to the
  ...
  return {
    ...
    revokeConnectionGrant: async (
      ...
      if (
        ...
        // profiles intentionally share one Zero-owned client, so revoking
      ...
      const drafts = Object.entries(mcpServers).map(([name, rawServer]) => {
        ...
        if (
          ...
          const credentialFields = Object.keys(headers)
            ...
            .map((key) => {
              warnings.push(
                `Header ${key} will be stored as a Zero secret before activation.`,
              );
              return {
        ...
        if (typeof server.command === "string") {
          warnings.push(
            "Imported stdio commands stay draft-only unless mapped to an approved Zero template.",
          );
          return {
            name,
