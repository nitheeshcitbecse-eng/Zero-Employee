...
function assertImportedSkillKeyAllowed(skill: ImportedSkill) {
  ...
  throw unprocessable(
    `Reserved Zero skill key "${skill.key}" cannot be imported from unbundled sources.`,
    {
      skillKey: skill.key,
...
const BUILT_IN_SKILL_TEST_RUN_TEMPLATE_BODY = [
  ...
  "",
  "Invoke and use the selected skill under test: `{{skillInvocation}}`. Use the pinned skill revision supplied by Zero as the source of truth, regardless of any other runtime skills.",
  "",
...
function builtInSkillTestRunTemplate(companyId: string): CompanySkillTestRunTemplate {
  return {
    ...
    description: "Zero's read-only default harness instructions for Skills Studio runs.",
...
function buildMissingRuntimeSourceDetail(skill: Pick<CompanySkill, "name" | "sourceLocator" | "metadata">) {
  ...
  if (sourcePath) {
    return `Company skill "${skill.name}" is in the library, but Zero cannot find its local source at ${sourcePath}.`;
  }
  return `Company skill "${skill.name}" is in the library, but Zero cannot find a valid local runtime source for it.`;
}
...
/**
 * A rename target must be a true Zero-managed local skill: a `local_path`
...
function deriveSkillSourceInfo(skill: SkillSourceInfoTarget): {
  ...
  if (metadata.sourceKind === "paperclip_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Zero skills are read-only.",
      sourceLabel: "Zero bundled",
...
export function companySkillService(db: Db) {
  ...
  async function assertLocalImportSourceAllowed(companyId: string, source: string) {
    ...
    if (!allowed) {
      throw forbidden("Local skill source is outside approved company workspace roots", {
        ...
        remediation: "Import from a configured Zero workspace or the company managed-skill directory.",
      });
    }
  }
  ...
  async function renameSkill(
    ...
    if (!isPaperclipManagedRenameTarget(skill)) {
      throw unprocessable(
        "Only Zero-managed skills can be renamed. Catalog, external, project-scanned, and unmanaged local skills are read-only.",
  ...
  async function installFromCatalog(
    ...
    const values = {
      ...
      authorName: storeMetadata.authorName ?? existingByKey?.authorName ?? "Zero",
