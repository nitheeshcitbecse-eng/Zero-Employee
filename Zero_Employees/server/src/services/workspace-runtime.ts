...
export type WorkspaceRuntimeExposureDeps = ExposureManagerDeps & {
   ...
   * automatic default on broker availability is what keeps a Zero install
 ...
 *
 *  - `auto` (default): eligible Zero-managed worktree runtimes get
 ...
 *
 * Intentionally narrow: only the Zero-managed dev runtime. Unmanaged and
 * custom external services are left exactly as they are, because the broker
 * only publishes allowlisted loopback ports it can prove Zero owns and we
 * do not want to relocate a service somebody else addresses by port.
 *
 * A *pinned* port is still a candidate. The pre-feature Zero App template
 ...
 * startup-reconciliation suites use this to simulate a Zero restart, where
...
export async function resetRuntimeServicesForTests(
  ...
  for (const record of runtimeServicesById.values()) {
    ...
    if (opts.simulateSupervisorExit) {
      ...
      // Zero keeping an anonymous stdio peer alive.
...
export function sanitizeRuntimeServiceBaseEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  // Zero instance. Runtime/service overrides are merged back after sanitizing.
...
function explainGitWorktreeBranchIncoherence(input: {
  ...
  if (!input.expectedHeadSha || !input.actualHeadSha) {
    return `Zero could not determine branch ancestry because the recorded branch "${input.expectedBranchName}" or checked-out branch "${actualBranch}" is missing a resolvable HEAD commit.`;
  }
  if (input.sameHead) {
  ...
  if (input.ancestryVerdict === "diverged") {
    return `The recorded branch "${input.expectedBranchName}" is not an ancestor of the checked-out branch "${actualBranch}", so Zero cannot prove a forward-only reconciliation.`;
  }
  return `Zero could not determine whether the checked-out branch "${actualBranch}" is forward of the recorded branch "${input.expectedBranchName}".`;
}
...
export async function ensureGitWorktreeBranchCoherent(input: {
  ...
  if (
    ...
    const reason = evidence.provenance.expectedBranchExists
      ...
      : "Automatic forward reconciliation: the recorded branch no longer exists, so Zero adopted the clean checked-out branch.";
  ...
  if (
    ...
    return {
      ...
      reconciledForward: false,
      warnings: [
        `${warningPrefix} The checked-out branch contains the recorded branch plus newer commits, so Zero adopted it for subsequent runs.`,
      ],
    };
  }
  ...
  if (
    ...
    return {
      ...
      reconciledForward: false,
      warnings: [
        `${warningPrefix} The detached HEAD contained the recorded branch plus newer commits, so Zero moved the recorded branch to that HEAD.`,
      ],
    };
  }
...
/**
 * Rows Zero reports stopped/removed whose reserved pair is still live on
...
/** Zero-owned Serve mappings, or null when the broker cannot be read. */
...
/**
 * Resolve the low-priority public URL hint injected into a managed Zero dev
...
async function spawnLocalRuntimeService(input: StartLocalRuntimeServiceInput): Promise<LocalRuntimeServiceStart> {
  ...
  // only way a legacy pinned port (the Zero App template's 45439) can be
  ...
  // Managed Zero worktrees are development environments, so their UI
  ...
  // the Zero dev runtime whether or not it is HTTPS-exposed, because the
  ...
  if (exposureConfig) {
    // Zero dev-runtime-specific hardening. Other managed processes are
  ...
  try {
    child = spawn(shell, ["-lc", command], {
      ...
      // Zero (or this parent handle below) cannot strand a request logger
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Zero", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
