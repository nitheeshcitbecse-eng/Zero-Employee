...
const DEFAULT_LOCAL_ENVIRONMENT_DESCRIPTION =
  "Default execution environment for Zero runs on this machine.";
...
export function environmentService(db: Db) {
  /**
   * Idempotently ensure THE Zero-managed sandbox environment for this
  ...
  const ensureManagedSandboxEnvironment = async (
    ...
    const reconciliation = await db.transaction(
      async (tx): Promise<ManagedSandboxEnvironmentReconcileResult> => {
        ...
        //   row that was never Zero-managed (the fallback lookup above).
        ...
        //   Zero's own provider-unavailability archival, so that path
        ...
        if (stockStatus === "operator_modified") {
          ...
          // an operator edit. If the binding records that Zero archived
  ...
  /**
   * Archive the Zero-managed sandbox row when its provider became
  ...
  const archiveManagedSandboxEnvironment = async (
    ...
    const archived = await db.transaction(async (tx) => {
      ...
      // Archival is a Zero-owned availability transition. Record only
