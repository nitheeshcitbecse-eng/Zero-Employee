    ...
    if (remote) {
      ...
      if (!unusedAdmission && !cancelledStartup) {
        ...
        if (!run.processPid && !run.processGroupId &&
            !await hasNativeLocalProcessStop(db, companyId, run.id)) return blocked("process_identity_missing", "The previous run has no verified stop record. Zero cannot start this message yet.");
