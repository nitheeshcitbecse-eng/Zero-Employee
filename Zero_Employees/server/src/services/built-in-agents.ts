...
const FALLBACK_REFLECTION_COACH_INSTRUCTIONS = [
  "# Reflection Coach",
  "",
  "You are Zero's built-in Reflection Coach.",
  ...
  "Do not apply changes in the same run. Present a reviewable diff and wait for the required Zero issue-thread approval before any follow-up applies it.",
  "",
].join("\n");
...
const FALLBACK_SUMMARIZER_INSTRUCTIONS = [
  "You are Summarizer, a built-in reporting agent at Zero.",
  "",
  "Turn the current state of a Zero scope (project, workspaces overview, project workspace, or execution workspace) into a short, honest, human-readable Markdown summary and write it back to that scope's summary slot as a new revision. Use the `summarize-status` skill as your operating procedure.",
  "",
...
const DEFINITIONS = validateBuiltInAgentDefinitions([
  {
    ...
    defaultInstructions:
      "You are Zero's built-in Briefs agent. Produce concise, sourced operational briefs that help the board understand current company work, risks, and next actions.",
    defaultRole: "general",
  ...
  },
  {
    ...
    defaultInstructions:
      "You are Zero's built-in Learning agent. Extract durable lessons from completed work, preserve useful patterns, and keep learning artifacts grounded in source context.",
    defaultRole: "general",
    allowedAdapterTypes: ["codex_local", "claude_local", "gemini_local", "opencode_local", "process"],
    defaultBudgetMonthlyCents: 0,
  },
