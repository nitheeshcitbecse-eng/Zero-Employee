...
/** Public Zero provider ids. The Teams SDK name remains an internal detail. */
...
/** Chat SDK-normalized inbound message plus Zero endpoint identity. */
...
/**
 * Provider-neutral callbacks consumed by the Zero control-plane service.
...
const OFFICIAL_TEAMS_CONNECTOR_HOST_SUFFIXES = [
  ...
  // that Zero's commercial-cloud-only credential flow supports every
...
function installDiscordNativeCommands(
  ...
  const denied =
    "This command is not available here. Open the Zero task or ask an operator to check your chat access.";
  const unconfirmed =
    "This command could not be confirmed. Check the Zero task before trying again.";
...
function assertDiscordAdapterCompatibility(adapter: Adapter, chat: Chat): void {
  ...
  if (discord.paperclipCompatibilityRevision !== "paperclip-discord-v6") {
    throw new DiscordAdapterCompatibilityError(
      "Zero patch revision paperclip-discord-v6 is unavailable",
    );
  }
...
export function scopeMicrosoftTeamsEgress(
  ...
  // members or Graph APIs before Chat dispatches to Zero's reach and
  ...
  switch (config.provider) {
    ...
    case "slack": {
      const adapterConfig: SlackAdapterConfig = {
        ...
        // Zero's durable publication outbox owns retry timing and
    ...
    case "github": {
      ...
      // exchange or API request could outlive Zero's publication lease
    ...
    case "discord": {
      ...
      const adapterConfig: DiscordAdapterConfig = {
        ...
        // Zero receives Discord messages and interactions over its
 ...
 * application callback runs. That ordering is unsafe for Zero: if the
 * `chat_deliveries` insert fails, a provider retry can be discarded by the SDK
 * even though Zero never durably accepted it. Zero's delivery ledger
    ...
    chat.onAction(async (event) => {
      ...
      if (!acceptsProviderScope(event.raw)) {
        if (transport === "discord_gateway") {
          ...
          // that never reached Zero's scoped action authorization.
          throw Object.assign(
...
/** One isolated Chat instance and provider adapter for one Zero endpoint. */
export class ChatSdkEndpointRuntime {
  ...
  constructor(options: CreateChatSdkEndpointRuntimeOptions) {
    ...
    this.chat = new Chat({
      ...
      // Zero acknowledges only after its own durable ledger write and
  ...
  async handleWebhook(
    ...
    const retryableTimeout = () =>
      new Response("Zero could not durably accept the event in time", {
        status: 503,
        headers: {
    ...
    return await this.webhookIngress.run(attempt, async () => {
      ...
      if (response.ok && attempt.callbackError !== undefined) {
        return new Response("Zero could not durably accept the event", {
          status: 503,
          headers: {
            "content-type": "text/plain; charset=utf-8",
            "retry-after": "1",
   ...
   * separately from the parsed Message, so without this bridge Zero
...
/** Process-local lifecycle registry; durable state remains in Zero persistence. */
