export type ChatPublicationErrorDisposition =
  | {
      kind: "retry";
      retryAfterMs: number;
      /** True when the provider explicitly asked Zero to slow down. */
...
export function classifyChatPublicationError(
  ...
  if (
    ...
    // Zero rejected the destination locally before opening a provider
  ...
  // Zero. An operator must inspect the native conversation before replay.
