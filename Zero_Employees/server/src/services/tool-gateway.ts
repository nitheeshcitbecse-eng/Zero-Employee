...
const BUILTIN_TOOLS: ToolGatewayDescriptor[] = [
  ...
  },
  {
    ...
    displayName: "List my Zero issues",
    description:
      "Zero self-MCP read fixture that lists the authenticated agent's current issues.",
    parametersSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Zero self-MCP read fixture that returns scoped issue context and plan document metadata.",
    parametersSchema: {
      type: "object",
...
const VIRTUAL_SEARCH_TOOLS: ToolGatewayDescriptor = {
  ...
  description:
    "Search the tools available through this Zero gateway without loading every target tool into the tool list.",
  parametersSchema: {
    type: "object",
    properties: {
      query: { type: "string" },
...
const VIRTUAL_RUN_TOOL: ToolGatewayDescriptor = {
  ...
  description:
    "Run a target tool by name after Zero applies the target tool's profile, policy, approval, and rate-limit checks.",
  parametersSchema: {
    type: "object",
    properties: {
      tool: { type: "string" },
...
export function createToolGatewayService(
  ...
  async function executeBuiltinTool(
    ...
    if (tool.name === "paperclip-self:list_my_issues") {
      if (!session.agentId) {
        throw new ToolGatewayHttpError(
          403,
          "Zero self tools require an agent-scoped gateway session",
  ...
  async function maybeRefreshPaperclipCloudGrant(
    ...
    if (oauth.strategy === "paperclip_id_connector") {
      // Zero ID used different endpoints, signing metadata, envelope
    ...
    const payload = {
      ...
      detailsMarkdown:
        grantKind === "organization"
          ...
          : "This run needs your personal authorization. Zero will not use another user's identity.",
      target: {
  ...
  async function requestElicitationForRecordedToolCall(input: {
    ...
    await db
      ...
      .set({
        ...
        errorMessage:
          "Remote MCP tool requested elicitation; Zero created an issue interaction for the response.",
        updatedAt: now,
      })
    ...
    return [
      {
        ...
        notes: ["Use the full Zero origin before the endpoint path."],
      },
      {
