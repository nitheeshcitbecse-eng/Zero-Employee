...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Zero will import the routine trigger without those limits.`);
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  // Bundled Zero skills ship with every build and may contain executable
...
function buildManifestFromPackageFiles(
  ...
  if (bundleSchemaVersion > BUNDLE_SCHEMA_VERSION) {
    throw unprocessable(`Company package declares schemaVersion ${bundleSchemaVersion}, which was produced by a newer Zero; this board reads up to schemaVersion ${BUNDLE_SCHEMA_VERSION}.`);
  }
