...
export function parseManagedConfigEnv(env: ManagedConfigEnv): ManagedInstanceConfig | null {
  ...
  if (doc.environments !== undefined) {
    ...
    // The DB enforces at most ONE Zero-managed sandbox row per instance
    ...
    if (doc.environments.length > 1) {
      fail(
        `"environments" supports at most one entry: each entry provisions the single Zero-managed sandbox environment (DB invariant environments_managed_sandbox_idx)`,
      );
    }
