...
class PaperclipRouteSemanticPort {
  ...
  private post(path: string, body: unknown) {
    return request(this.app).post(path).set("X-Zero-Run-Id", this.binding.runId).send(body);
  }

  private put(path: string, body: unknown) {
    return request(this.app).put(path).set("X-Zero-Run-Id", this.binding.runId).send(body);
  }

  private patch(path: string, body: unknown) {
    return request(this.app).patch(path).set("X-Zero-Run-Id", this.binding.runId).send(body);
  }
}
...
function createProductionAuthorityApp(db: Db, ids: PaperclipSemanticConformanceIds): Application {
  ...
  app.use((req, _res, next) => {
    req.actor = {
      ...
      agentId: ids.actorId,
      companyId: ids.companyId,
      runId: req.header("X-Zero-Run-Id") ?? undefined,
      source: "agent_jwt",
    };
    next();
  });
