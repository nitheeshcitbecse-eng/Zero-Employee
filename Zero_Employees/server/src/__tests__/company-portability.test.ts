...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "paperclip",
        name: "paperclip",
        description: "Zero coordination skill",
        markdown: "---\nname: paperclip\ndescription: Zero coordination skill\n---\n\n# Zero\n",
        sourceType: "github",
  ...
  it("exports referenced skills as stubs by default with sanitized Zero extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Zero"');
  ...
  it("exports agent permission grants through the Zero extension and manifest", async () => {
    const db = {
  ...
  it("exports default sidebar order into the Zero extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/paperclipai/paperclip/paperclip/SKILL.md"])).toContain("# Zero");
    expect(asTextFile(exported.files["skills/paperclipai/paperclip/paperclip/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/paperclipai/paperclip/paperclip/references/api.md"])).toContain("# API");
  });

  it("exports catalog skill provenance in portable Zero frontmatter", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("normalizes invalid imported project icon names to null", async () => {
    ...
    await portability.importBundle({
      ...
      target: { mode: "new_company", newCompanyName: "Imported Zero" },
      collisionStrategy: "rename",
    }, "user-1");
  ...
  it("exports routines as recurring task packages with Zero routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("suffixes a manifest-derived company name that collides with an existing company", async () => {
    ...
    companySvc.create.mockResolvedValue({
      id: "company-imported",
      name: "Imported Zero (3)",
    });
    accessSvc.ensureMembership.mockResolvedValue(undefined);

    const files = {
      "COMPANY.md": ["---", 'schema: "agentcompanies/v1"', 'name: "Imported Zero"', "---", ""].join("\n"),
    };
  ...
  it("skips name de-duplication for agent-safe imports so collisions stay unobservable", async () => {
    ...
    companySvc.list.mockResolvedValue([{ name: "Imported Zero" }]);
    companySvc.create.mockResolvedValue({
      id: "company-imported",
      name: "Imported Zero",
    });
  ...
  it("imports recurring tasks without a project or assignee as paused routines", async () => {
    ...
    const request = {
      ...
      target: { mode: "new_company" as const, newCompanyName: "Imported Zero" },
  ...
  it("imports a vendor-neutral package without .paperclip.yaml", async () => {
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Zero");
  ...
  it("rejects packages produced by a newer Zero", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    await expect(portability.importBundle({
      ...
      agents: "all",
      collisionStrategy: "rename",
    }, "user-1")).rejects.toThrow(/newer Zero/);
    expect(issueSvc.importIssues).not.toHaveBeenCalled();
  });
    ...
    issueSvc.listComments.mockResolvedValue([
      {
        ...
        authorAgentId: null,
        authorUserId: null,
        body: "Zero needs a disposition before this issue can continue.",
        presentation,
        metadata,
...
describe("dedupeImportedCompanyName", () => {
  it("returns the base name when nothing collides", () => {
    expect(dedupeImportedCompanyName("Zero", ["Other Co"])).toBe("Zero");
    expect(dedupeImportedCompanyName("Zero", [])).toBe("Zero");
  });

  it("suffixes past every taken candidate, case-insensitively", () => {
    expect(dedupeImportedCompanyName("Zero", ["paperclip"])).toBe("Zero (2)");
    expect(dedupeImportedCompanyName("Zero", ["Zero", "Zero (2)"])).toBe("Zero (3)");
    expect(dedupeImportedCompanyName("Zero", ["PAPERCLIP", "paperclip (2)"])).toBe("Zero (3)");
  });

  it("ignores surrounding whitespace in existing names", () => {
    expect(dedupeImportedCompanyName("Zero", ["  Zero  "])).toBe("Zero (2)");
  });
});
