...
async function createTempRepo(defaultBranch = "main") {
  ...
  await runGit(repoRoot, ["config", "user.name", "Zero Test"]);
...
describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited Zero and pnpm auth flags before spawning runtime services", () => {
...
describe("realizeExecutionWorkspace", () => {
  ...
  it("writes an isolated repo-local Zero config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
  ...
  it("regenerates stale worktree config that points at another host", async () => {
    ...
    try {
      ...
      expect(result.stderr).toContain("Existing isolated Zero worktree config is stale for this host; regenerating.");
...
describe("ensureRuntimeServicesForRun", () => {
  ...
  it("enables UI dev middleware by default for managed Zero worktree runtimes", async () => {
  ...
  it("requires Zero dev runtime services to pass /api/health readiness", async () => {
  ...
  it("replaces a reused Zero dev runtime whose 2xx health payload is unhealthy", async () => {
  ...
  it("reuses a shared Zero dev runtime after one transient unhealthy response", async () => {
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    ...
    // A Zero dev runtime must answer `/api/health` semantically before it may
  ...
  it("does not leak parent Zero instance env into runtime service commands", async () => {
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  it("restores desired services when one row is stopped and a live registered service has no row", async () => {
    ...
    const actor = { id: null, name: "Zero", companyId };
    const workspace = {
      ...buildWorkspace(workspaceRoot),
      projectId,
      workspaceId: projectWorkspaceId,
  ...
  it("backfills a pre-existing HTTP-only managed worktree runtime to verified HTTPS in place", async () => {
    ...
    try {
      ...
      // ---- Deploy: the feature turns on and Zero restarts. ----
      await resetRuntimeServicesForTests();
