...
describe("claude local skill sync", () => {
  ...
  it("keeps the operational Zero skill configured when no explicit selection exists", async () => {
  ...
  it("normalizes legacy flat Zero skill refs to canonical keys", async () => {
    const snapshot = await listClaudeSkills({
      agentId: "agent-3",
      companyId: "company-1",
      adapterType: "claude_local",
  ...
  it("shows host-level user-installed Claude skills as read-only external entries", async () => {
    ...
    expect(snapshot.entries).toContainEqual(expect.objectContaining({
      ...
      readOnly: true,
      detail: "Installed outside Zero management in the Claude skills home.",
    }));
  });
});
