...
describeEmbeddedPostgres("companySkillService.list", () => {
  ...
  async function createManagedSkillDir(companyId: string, prefix: string) {
    if (!paperclipHome) throw new Error("Expected Zero test home");
  ...
  it("does not retouch unchanged bundled skills during list refresh", async () => {
    ...
    if (!bundledSkill) throw new Error("Expected bundled Zero skills fixture");
  ...
  it("seeds bundled skill releases idempotently and materializes the frozen champion snapshot", async () => {
    ...
    if (!paperclipSkill) throw new Error("Expected bundled Zero skill");
  ...
  it("resolves detail by unique skill slug for Studio deep links", async () => {
    ...
    const skill = await svc.createLocalSkill(companyId, {
      name: "Zero Blog Cover Image",
      slug: "paperclip-blog-cover-image",
      markdown: "# Zero Blog Cover Image\n",
    });
  ...
  it("rejects unbundled package imports that claim reserved Zero skill keys", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    ...
    await expect(svc.importPackageFiles(companyId, {
      ...
      status: 422,
      message: 'Reserved Zero skill key "paperclipai/paperclip/paperclip" cannot be imported from unbundled sources.',
    });
    ...
    expect(stored?.name).not.toBe("Trojan Zero");
    expect(stored?.markdown).not.toContain("Trojan Zero");
  });
  ...
  it("renames a Zero-managed skill, moving the directory and rewriting SKILL.md frontmatter", async () => {
    const companyId = randomUUID();
  ...
  it("rejects renaming non Zero-managed skill sources with 422", async () => {
    const companyId = randomUUID();
    await seedCompany(companyId);
