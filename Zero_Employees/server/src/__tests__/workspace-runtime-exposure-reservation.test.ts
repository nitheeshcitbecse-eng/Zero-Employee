 ...
 * local Zero instance's on-disk service registry still names, even with
...
/**
 * The real, unredirected Zero home directory. `beforeEach` later points
...
/**
 * Every port a local Zero instance's on-disk service registry currently
 * records, read once per case from the real, unredirected Zero home. A
...
(embeddedPostgresSupport.supported ? describe : describe.skip)(
  ...
  () => {
    ...
    beforeEach(async () => {
      ...
      // Zero instance on this host and could confuse a live server.
    ...
    async function seedIncident(options?: { leaseStatus?: string; leasedRowStatus?: string }) {
      ...
      await db.insert(projects).values({ id: projectId, companyId, name: "Zero App", status: "in_progress" });
    ...
    function startInput(seed: Awaited<ReturnType<typeof seedIncident>>, executionWorkspaceId: string) {
      return {
        ...
        actor: { id: null, name: "Zero", companyId: seed.companyId },
        issue: null,
        db,
        workspace: {
