...
describe("teamsCatalogService", () => {
  beforeEach(() => {
    ...
    mockCompanyPortabilityService.importBundle.mockResolvedValue({
      company: { id: "company-1", name: "Zero", action: "unchanged" },
      agents: [],
      projects: [],
      envInputs: [],
      warnings: [],
  ...
  it("preserves package-declared Zero sidecar permissions while adding generated catalog provenance", async () => {
    const svc = teamsCatalogService({} as any);
