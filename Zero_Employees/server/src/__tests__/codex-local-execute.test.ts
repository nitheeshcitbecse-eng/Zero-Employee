...
describe("codex execute", () => {
  it("uses a Zero-managed CODEX_HOME outside worktree mode while preserving shared auth and config", async () => {
    ...
    try {
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining("Using Zero-managed Codex home"),
        }),
      );
    } finally {
    ...
    try {
      ...
      expect(logs).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            stream: "stderr",
            chunk: expect.stringContaining("Zero cannot enforce policies for that direct entry"),
          }),
        ]),
      );
  ...
  it("emits a command note that Codex auto-applies repo-scoped AGENTS.md files", async () => {
    ...
    try {
      ...
      expect(commandNotes).toContain(
        "Codex exec automatically applies repo-scoped AGENTS.md instructions from the current workspace; Zero does not currently suppress that discovery.",
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("injects structured Zero wake payloads into env and prompt", async () => {
    ...
    try {
      ...
      expect(capture.prompt).toContain("## Zero Wake Payload");
...
`;
  ...
  it("uses safer invocation settings and a fresh-session handoff for codex transient fallback retries", async () => {
    ...
    try {
      ...
      expect(capture.prompt).toContain("Zero session handoff:");
  ...
  it.each([{ conversationMode: false, resumedSession: true }, { conversationMode: true, resumedSession: true }, { conversationMode: true, resumedSession: false }])("retains current task policy (conversation=$conversationMode, resumed=$resumedSession)", async ({ conversationMode, resumedSession }) => {
    ...
    try {
      ...
      expect(capture.prompt).toContain(resumedSession ? "## Zero Resume Delta" : "## Zero Wake Payload");
      expect(capture.prompt).toContain("Do not switch to another issue until you have handled this wake.");
      expect(capture.prompt).toContain("Second comment");
      ...
      if (resumedSession) {
        ...
        expect(invocationPrompt).toContain("## Zero Resume Delta");
