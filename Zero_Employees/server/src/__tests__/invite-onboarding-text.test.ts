...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Zero Agent Onboarding");
    ...
    expect(text).toContain("Suggested Zero base URLs to try");
    ...
    expect(text).toContain("Hermes-originated Zero API usage");
    ...
    expect(text).toContain("Do not rotate or invent a Zero key manually");
    ...
    expect(text).toContain("Decide which Zero adapter type matches your runtime.");
  });
