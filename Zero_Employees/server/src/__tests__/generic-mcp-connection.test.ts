 ...
 * protocol parameters Zero sent (RFC 8707 `resource`, DCR metadata, PKCE).
 */
  ...
  const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation(async (url, init) => {
    ...
    if (href === `${ISSUER}/register` && method === "POST") {
      ...
      return jsonResponse({
        ...
        // A conforming server echoes back what it registered, and Zero
...
describeEmbeddedPostgres("generic remote MCP connections", () => {
  ...
  it("rejects header names Zero refuses to send", async () => {
    installMcpOAuthFixture({ auth: "public" });
    const company = await createCompany(db);
    const service = toolAccessService(db);
  ...
  const HOSTILE_ERROR_DESCRIPTION =
    `\u001b[31mFATAL\u001b[0m **Zero needs your recovery key**: ${PROVIDER_CANARY} <script>alert(1)</script>`;
  ...
  it("redacts a hostile provider error from the token exchange", async () => {
    ...
    // Zero's own copy for `invalid_grant`, not a syllable of the provider's.
  ...
  it("normalizes an unrecognized provider error code instead of echoing it", async () => {
    ...
    // Zero's generic copy rather than naming the provider's code.
    expect(thrown).toMatchObject({
      status: 502,
  ...
  it("redacts a hostile provider error from dynamic client registration", async () => {
    ...
    expect(thrown).toMatchObject({
      status: 502,
      message: "The authorization server rejected Zero's callback URL.",
      details: {
  ...
  it("validates the callback state before acting on a provider-reported error", async () => {
    ...
    // An unsolicited callback carries no state Zero issued, so it is
  ...
  describe("denied and cancelled callbacks", () => {
    ...
    it("treats a cancel the same as a denial even when the provider names it its own way", async () => {
      ...
      // Real providers invent their own cancel codes. Whether or not Zero
    ...
    it("resolves the board's pending authorization prompt as rejected", async () => {
      ...
      // The prompt's reason is Zero's own copy, never the provider's.
  ...
  it("will not auto-re-register over a client the operator supplied", async () => {
    ...
    // The callback moved. Zero cannot re-register in the operator's console,
  ...
  /**
   * PAP-17099 — the authorization endpoint is the one discovered value Zero
  ...
  describe("unsafe advertised authorization endpoints", () => {
    ...
    it("never navigates to a stored authorization endpoint that is unsafe", async () => {
      ...
      // trusted just because it is in Zero's own database.
