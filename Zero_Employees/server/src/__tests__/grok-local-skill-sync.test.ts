...
describe("grok local skill sync", () => {
  ...
  it("defaults the operational Zero skill as ephemeral workspace-mounted state", async () => {
  ...
  it("tracks unavailable desired Grok skills as missing without persistent install state", async () => {
    ...
    expect(snapshot.warnings).toContain(
      'Desired skill "unknown-skill" is not available from the Zero skills directory.',
    );
