...
describeEmbeddedPostgres("secretService", () => {
  ...
  it("reports current-user secret rollback failures when AWS create cleanup cannot remove the reserved row", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockRejectedValueOnce(
      new SecretProviderClientError({
        ...
        rawMessage:
          "AccessDeniedException: arn:aws:sts::123456789012:assumed-role/prod/Zero cannot create secret",
      }),
    );
    ...
    await expect(
      ...
      status: 500,
      message: "Secret create failed and Zero could not roll back the local secret reservation.",
      details: {
  ...
  it("keeps a local cleanup handle when create rollback cleanup fails", async () => {
    ...
    await expect(
      ...
      status: 500,
      message: "Secret create failed and Zero could not clean up the remote provider secret.",
      details: {
  ...
  it("sanitizes AWS remote import preview provider errors before crossing the service boundary", async () => {
    ...
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Zero is not authorized to perform secretsmanager:ListSecrets";
  ...
  it("sanitizes AWS managed secret create failures and removes the reserved row", async () => {
    ...
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Zero is not authorized to perform secretsmanager:CreateSecret on arn:aws:secretsmanager:us-east-1:123456789012:secret:paperclip/prod-use1";
  ...
  it("sanitizes AWS provider vault discovery errors before crossing the service boundary", async () => {
    ...
    expect(thrown).toMatchObject({
      ...
      details: {
        ...
        region: "us-east-1",
        credentialPath: "Zero server runtime/provider credential path",
        requiredCapability: "secretsmanager:ListSecrets",
        actionableMessage:
          "AWS discovery preview needs secretsmanager:ListSecrets in the selected region for the Zero server runtime/provider credential path.",
        safeAlternative:
  ...
  it("sanitizes AWS remote import row provider errors", async () => {
    ...
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Zero is not authorized to perform secretsmanager:DescribeSecret on arn:aws:secretsmanager:us-east-1:123456789012:secret:prod/openai";
  ...
  it("rejects Zero-managed AWS namespace refs during preview and import commit", async () => {
    const companyId = await seedCompany();
    const svc = secretService(db);
    const awsVault = await svc.createProviderConfig(companyId, {
      provider: "aws_secrets_manager",
    ...
    expect(result.results[0]?.reason).toMatch(/Zero-managed namespace/i);
