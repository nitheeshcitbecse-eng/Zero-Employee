...
describe("buildAgentOnboardingPrompt", () => {
  it("includes Hermes Gateway setup guidance for external agents", () => {
    ...
    expect(prompt).toContain("not the Zero API key");
    ...
    expect(prompt).toContain("Zero maps it to `/api`");
    ...
    expect(prompt).toContain("`hermes_local` runs Hermes on the Zero host");
    expect(prompt).toContain("Hermes-originated Zero API calls");
    expect(prompt).toContain("Do not rotate or invent a Zero key manually");
