...
export function breakdownSummarySentence(
  ...
  const parts: string[] = [
    `Zero will create one ${noun} per item in ${names.targetPipelineName} → ${names.entryStageName}`,
  ];
...
/**
 * The read-only "Zero handles this" mechanics bullets, composed from the
