    ...
    createdRoot.render(
      <TooltipProvider>
        <TrustPresetSection
          ...
          onChange={onChange}
          companyId="company-1"
          projectCandidates={[{ id: "project-1", label: "Zero App" }]}
...
describe("TrustPresetSection", () => {
  it("hides the CE boundary editor under Standard", () => {
    ...
    expect(view.text()).not.toContain("Get Zero EE.");
  });
  ...
  it("shows a selectable CE boundary editor for low-trust review", () => {
    ...
    expect(view.text()).toContain("Zero App");
    expect(view.text()).toContain("Get Zero EE.");
