...
const EXACT_ACTIONS: Record<string, ExactAction> = {
  ...
  generic_api_request: { action: "request", running: "Calling the Zero API", completed: "Called the Zero API" },
};
...
function paperclipSummaryGroup(action: Action): ToolSummaryGroup {
  if (action === "read" || action === "list") return group("paperclip_read", "Zero read", "Zero reads");
