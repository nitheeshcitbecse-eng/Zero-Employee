...
function applyObservabilityHeaders(headers: Headers) {
  if (headers.has("X-Zero-Tab-Visible")) return; // caller override wins
  const visibility = getPageVisibility();
  headers.set("X-Zero-Tab-Visible", getVisibilityHeaderValue(visibility));
  if (typeof window !== "undefined" && window.location) {
    headers.set("X-Zero-Route", window.location.pathname);
  }
}
