...
export function CodexLocalConfigFields({
  ...
  // `runnerManaged` already does for the Zero Runner.
  ...
  const fastModeMessage = fastModeManualModel
    ...
    : fastModeSupported
      ...
      : `Fast mode currently only works on ${supportedModelsLabel} or manual model IDs. Zero will ignore this toggle until the model is switched.`;
    ...
    <>
      ...
      {runnerManaged && runnerProvider === "aws_agentcore" && (
        <>
          ...
          <Field
            ...
            hint="Zero estimate; AWS does not provide a per-session currency hard stop."
          >
            <DraftNumberInput
      ...
      {runnerManaged && runnerPermissionCapability.configurable && (runnerPermissionCapability.options.length > 1 || runnerPermissionModeUnsupported) && (
        <Field
          ...
          hint={`${runnerPermissionCapability.description} The selected mode does not widen Zero's workspace, network, credential, or planning boundaries.`}
        >
          <Select
            value={
          ...
          {runnerPermissionModeUnsupported && runnerProvider === "codex" && (
            <p className="mt-1 text-xs text-destructive" role="alert">
              This saved Codex mode cannot start or recover a Zero Runner
        ...
        <>
          ...
          {!managedSandboxOnly && (
            <Field
              ...
              hint="Optional ACP session state directory. Defaults to Zero-managed organization/agent scoped storage."
            >
