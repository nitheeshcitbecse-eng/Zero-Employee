...
describe("genericConnectGuidance", () => {
  ...
  it("points at the deployment when Zero itself has no public HTTPS address", () => {
  ...
  it("does not ask the operator to resolve an internal name conflict", () => {
    ...
    expect(guidance).toMatchObject({ title: "Zero couldn’t name this connection", focus: "none" });
  ...
  it("passes a rejected header's own message through", () => {
    const guidance = genericConnectGuidance(
      "mcp_header_rejected",
      'Zero manages the "Host" header and cannot send a custom value for it.',
    );
...
describe("customHeaderError", () => {
  ...
  it("rejects headers Zero refuses to send", () => {
    expect(customHeaderError([{ id: "a", name: "Host", value: "evil.example" }]))
      .toContain('Zero manages the "Host" header');
  });
