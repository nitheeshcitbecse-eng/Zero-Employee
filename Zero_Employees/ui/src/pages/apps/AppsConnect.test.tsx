...
const mockCompany = vi.hoisted(() => ({
  value: {
    ...
    selectedCompany: { id: "company-1", name: "Zero" } as { id: string; name: string } | null,
  },
}));
...
describe("AppsConnect — Connect with a link (M4 frame)", () => {
  ...
  beforeEach(() => {
    ...
    mockCompany.value = {
      ...
      selectedCompany: { id: "company-1", name: "Zero" },
    };
    mockSearch.value = "";
  ...
  it("shows Gmail's instance configuration notice instead of a dead connect path", async () => {
    const reason = "Configure Zero ID before connecting Gmail.";
    listGalleryMock.mockResolvedValue({
  ...
  it("collects customer-owned OAuth client details for a curated manual OAuth app", async () => {
    ...
    expect(container.textContent).toContain("Zero callback URL");
  ...
  it("prompts for one-time Cloud registration inside Gmail when managed methods are not yet advertised", async () => {
    ...
    expect(container.textContent).toContain("Connect with Zero");
    expect(container.textContent).toContain(
      "You must connect this instance to Zero to connect to Gmail (you only need to do this once).",
    );
    expect(buttonByText("Connect with Zero")?.closest(".rounded-xl")?.classList.contains("border-border")).toBe(true);
    ...
    await act(async () => {
      buttonByText("Connect with Zero")?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });
    await flushReact();
  ...
  it.each([false, true])("retains task access and interaction through enrollment (popup blocked: %s)", async (popupBlocked) => {
    ...
    await passAccessStep();
    await act(async () => buttonByText("Connect with Zero")?.click());
    await flushReact();
    ...
    expect(startCloudConnectorEnrollmentMock).toHaveBeenCalledWith("company-1", "Zero", "/apps/connect?source=gmail&stage=setup&intent=intent-1&enrollment_host=dialog");
    ...
    expect(container.textContent).toContain("What should Zero be able to do?");
  ...
  it.each(["2020-01-01T00:00:00.000Z", "2099-01-01T00:00:00.000Z"])(
    ...
    async (expiresAt) => {
      ...
      expect(startCloudConnectorEnrollmentMock).toHaveBeenCalledWith(
        "company-1", "Zero", "/apps/connect?source=github&stage=setup",
      );
      expect(navigateTopLevelMock).toHaveBeenCalledWith(
        "https://my-staging.paperclip.app/connections/enroll?id=enroll-test",
      );
  ...
  it("restores the setup step after the one-time enrollment callback", async () => {
    ...
    expect(container.textContent).not.toContain("Connect with Zero");
  });
  ...
  it("keeps GitHub sign-in intent when enrollment recovery reveals an unavailable profile", async () => {
    ...
    expect(container.textContent).toContain("Zero couldn’t check Cloud registration. Try again.");
    expect(container.textContent).not.toContain("Your GitHub key");
  ...
  it.each([...new Set(Object.values(GOOGLE_WORKSPACE_CONNECTOR_PROFILES).map((profile) => profile.appSlug))]
    ...
    async ({ slug, enrollmentReturn }) => {
      ...
      expect(buttonByText("Continue to sign in")?.disabled).toBe(true);
      const managedAuth = buttonByText("Use Zero instead");
  ...
  it("never renders self-host enrollment when the connector identity is already active", async () => {
    ...
    expect(container.textContent).not.toContain("You must connect this instance to Zero");
    expect(container.textContent).not.toContain("Connect with Zero");
  });
  ...
  it("keeps Google Drive prerequisites off access and defaults to its write-capable method", async () => {
    ...
    expect(container.textContent).not.toContain("does not enable unrelated Zero customers");
  ...
  it("renders Google Calendar as one minimal, unboxed setup screen", async () => {
    ...
    const capabilityQuestion = Array.from(container.querySelectorAll("label")).find(
      (label) => label.textContent === "What should Zero be able to do?",
    );
  ...
  it("shows an in-flight state while Zero prepares Notion sign-in", async () => {
    mockSearch.value = "source=notion";
    listGalleryMock.mockResolvedValueOnce({ apps: [NOTION] });
  ...
  it("shows installation recovery for GitHub even when an advanced PAT method is available", async () => {
    ...
    await render();
    await flushReact();
    expect(container.textContent).toContain("Install Zero and grant at least one repository");
    expect(container.querySelector('a[href="https://github.com/apps/paperclip-for-github/installations/new"]')?.textContent).toBe("Install Zero on GitHub");
    expect(container.textContent).not.toContain("Your GitHub key");
    await act(async () => buttonByText("Try again")!.click());
    await flushReact();
  ...
  it.each(["connection", "application"] as const)(
    ...
    async (failedLookup) => {
      ...
      expect(container.textContent).not.toContain("Couldn’t load connection setup");
      expect(container.textContent).toContain("Connect Notion to Zero");
    },
  );
...
describe("AppsConnect — guided generic MCP flow (PAP-17087)", () => {
  ...
  it("offers a curated setup as a convenience without leaving the generic flow", async () => {
    ...
    expect(container.textContent).toContain("Zero has a guided setup for Zapier.");
    expect(container.textContent).toContain("Connect your own MCP server");
  ...
  it("uses deployment guidance without rendering the server env-var message", async () => {
    ...
    expect(container.textContent).toContain("This Zero needs a public HTTPS address first");
  ...
  it("does not ask the operator to resolve an internal name conflict", async () => {
    ...
    expect(container.textContent).toContain("Zero couldn’t name this connection");
  ...
  it("blocks a header Zero refuses to send before making a request", async () => {
    await render();
    await gotoLinkFrame(container, "https://mcp.example.test/mcp");
    await openAdvanced();
    await act(async () => {
    ...
    expect(container.textContent).toContain('Zero manages the "Host" header');
