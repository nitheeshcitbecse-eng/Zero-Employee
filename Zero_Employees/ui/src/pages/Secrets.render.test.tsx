...
describe("Secrets page layout", () => {
  ...
  it("warns that removing a provider vault only removes Zero config", async () => {
    ...
    expect(document.body.textContent).toContain("from Zero only");
    ...
    const confirmButton = [...document.querySelectorAll("button")].find(
      (button) => button.textContent?.includes("Remove from Zero"),
    ) as HTMLButtonElement | undefined;
    await act(async () => {
      confirmButton?.click();
    });
  ...
  it("explains AWS managed secret creation failures with actionable safe details", async () => {
    const rawProviderMessage =
      "AccessDeniedException: arn:aws:sts::123456789012:assumed-role/prod/Zero is not authorized";
    mockSecretsApi.create.mockRejectedValueOnce(
      new ApiError("AWS Secrets Manager denied the request. Check IAM permissions for this provider vault.", 403, {
        details: {
          ...
          safeAlternative:
            "If the secret already exists in AWS, link it as an external reference instead of creating a Zero-managed value.",
        },
      }),
    );
    const root = createRoot(container);
  ...
  it("shows AWS discovery errors without replacing manual vault form values", async () => {
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Zero is not authorized";
    ...
    expect(errorBanner?.textContent).toContain("Zero server runtime/provider credential path");
