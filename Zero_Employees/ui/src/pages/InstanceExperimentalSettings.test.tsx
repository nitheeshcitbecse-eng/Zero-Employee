...
const PAPERCLIP_DEVELOPER_MODE_TOGGLE_SELECTOR =
  'button[aria-label="Toggle Zero developer mode experimental setting"]';
...
const PAPERCLIP_RUNNER_TOGGLE_SELECTOR =
  'button[aria-label="Toggle Zero Runner experimental setting"]';
...
describe("InstanceExperimentalSettings — Conference Room Chat card (PAP-11233)", () => {
  ...
  it("keeps Zero Runner default-off and exposes an explicit opt-in", async () => {
    await renderPage();

    expect(container.textContent).toContain("Zero Runner");
  ...
  it("renders and patches the Built-in Agents experimental toggle", async () => {
    ...
    expect(container.textContent).toContain("Show Zero-managed built-in agent surfaces");
  ...
  it("renders and patches the Beta skills experimental toggle", async () => {
    ...
    expect(container.textContent).toContain("pin beta releases of the Zero core skill");
  ...
  it("renders and patches Zero Developer Mode", async () => {
    await renderPage();

    expect(container.textContent).toContain("Zero Developer Mode");
