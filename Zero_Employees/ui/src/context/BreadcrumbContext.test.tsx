...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Zero", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Zero",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Zero");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Zero");
    expect(buildDocumentTitle([], null)).toBe("Zero");
  });
});
