 ...
 * so an exposed Zero dev runtime MUST bind `127.0.0.1`. The server used to
...
/**
 * A Zero dev-runner invocation — the only shape that understands
 * `--bind` / `--bind-host`.
 *
...
/**
 * Rewrite a Zero dev-runner command so it explicitly requests the loopback
