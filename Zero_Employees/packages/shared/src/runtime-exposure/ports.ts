 ...
 * and tests all agree on the dedicated Zero runtime port range and the
...
/**
 * Dedicated, allowlisted port range for managed Zero runtime APP
...
/**
 * Fixed offset between an app port and its Zero Vite HMR companion port.
 * Matches Zero dev mode's `server port + offset` HMR convention and keeps
...
/**
 * Derive Zero's Vite HMR companion port for any valid application port.
 *
 * Normal Zero instances can use ports outside the dedicated exposure
