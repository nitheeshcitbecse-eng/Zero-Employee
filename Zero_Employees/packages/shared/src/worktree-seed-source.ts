...
function readInstanceId(configPath: string, label: "source" | "target"): string {
  ...
  if (!existsSync(envPath)) {
    ...
    throw new Error(`Registered ${label} Zero config is missing its adjacent .env instance pointer.`);
  }
  ...
  throw new Error(`Registered ${label} Zero config has no PAPERCLIP_INSTANCE_ID binding.`);
}
...
function inspectDeclaredEntry(entryPath: string, configPath: string, detail?: string): Stats | null {
  try {
    ...
    throw new Error(
      `Registered base project workspace Zero config at ${configPath} cannot be inspected (${errorCode(error)}${detail ?? ""}).`,
    );
  }
}
...
export function baseWorkspaceDeclaresInstanceConfig(baseWorkspaceCwd: string): boolean {
  ...
  if (configDirEntry?.isSymbolicLink()) {
    try {
      ...
      throw new Error(
        `Registered base project workspace Zero config at ${configPath} cannot be inspected (${errorCode(error)} on its .paperclip symlink target).`,
      );
    }
  }
  return false;
...
export function resolveRegisteredWorktreeSeedSource(
  ...
  if (!selectedPath) {
    throw new Error(
      "Registered base project workspace has no Zero config of its own and no explicit source was provided.",
    );
  }
  const canonicalSourceConfigPath = canonicalRegularFile(selectedPath, "Registered source Zero config");
  if (registeredConfigPath && canonicalSourceConfigPath !== registeredConfigPath) {
    throw new Error("Registered source Zero config escapes the base project workspace or uses a symlink alias.");
  }

  if (explicitSource) {
    const canonicalExplicitSource = canonicalRegularFile(explicitSource, "Explicit source Zero config");
    if (canonicalExplicitSource !== canonicalSourceConfigPath) {
      throw new Error("Explicit source Zero config does not match the registered base project workspace.");
    }
  }

  const canonicalTargetConfigPath = canonicalRegularFile(
    input.targetConfigPath,
    "Target worktree Zero config",
  );
  if (canonicalSourceConfigPath === canonicalTargetConfigPath) {
    throw new Error("Source and target Zero configs are the same canonical file.");
  }
  ...
  if (targetInstanceId !== input.expectedTargetInstanceId) {
    throw new Error("Target Zero instance does not match the registered worktree instance.");
  }
  if (sourceInstanceId === targetInstanceId) {
    throw new Error("Source and target Zero configs name the same instance.");
  }
