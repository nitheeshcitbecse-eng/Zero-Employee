...
export const connectionGrantSchema = z.object({
  ...
  if (grant.externalCredential && grant.credentialSecretRefs.length > 0) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["credentialSecretRefs"], message: "External grants cannot also contain Zero secret references" });
  }
});
 ...
 *
 * `auto` is the default and the only value the simple path sends: Zero
 ...
 * it becomes a Zero secret ref and is never read back.
 */
...
/**
 * Reject `headers.*` credential paths whose header name Zero refuses to
