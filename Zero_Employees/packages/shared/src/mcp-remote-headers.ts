 ...
 * uses, and its value is stored as a Zero secret like every other one.
 */
...
/**
 * Headers Zero refuses to project from a user-supplied config.
 *
 ...
 * ambient browser-style credentials that Zero cannot scope or rotate.
...
/**
 * Is `name` a header Zero is willing to send on a user-configured remote
...
export function mcpRemoteHeaderRejectionMessage(
  ...
  switch (reason) {
    ...
    case "forbidden":
      return `Zero manages the "${headerName}" header and cannot send a custom value for it.`;
    case "value_too_long":
