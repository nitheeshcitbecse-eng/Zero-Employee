 ...
 *
 * The Zero extension keywords below are recognised by the Zero UI
 * but are otherwise ignored by standard JSON Schema validators.
 */
export type JsonSchema = {
  /**
   * When true, the Zero config UI hides this property behind an
...
export interface PluginEnvironmentDriverDeclaration {
  ...
  /**
   * How Zero should apply a captured template ref back into this provider's
...
/**
 * Declares a normal Zero agent that a plugin can provision and later
...
export interface PluginManagedAgentDeclaration {
  ...
  /** Suggested Zero runtime configuration. */
...
/**
 * Declares a normal Zero project that a plugin can provision and later
 * resolve by stable key within each company.
 */
...
/**
 * Lower-bound semver requirement for the Zero host.
 *
