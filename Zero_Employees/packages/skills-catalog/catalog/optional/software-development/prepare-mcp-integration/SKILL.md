...
description: >
  ...
  exact-revision human approval, and one governed Zero connector PR per
...
research PR in `paperclip-content`, then connector implementation in Zero
...
- Create one Zero App PR per approved connection. Shared prerequisite
...
- Request the broadest vendor permissions and scopes the connection can
  ...
  Zero's action catalog, resource boundaries, ask-first policies,
...
- Treat vendor permission breadth and Zero execution governance as
...
1. Load the current Zero skill for checkout, comments, interactions,
   durable state, and final disposition. Load the standard PR-preparation skill
   (`prepare-paperclip-pr` for Zero agents) before opening any PR.
2. Resolve the input URL(s), vendor/platform, intended MCP endpoint or API,
   target repositories and branches, and the Zero issue that owns the
...
6. Prefer official vendor documentation, protocol/RFC sources, safe live
   probes, and current Zero code. Use third-party sources only to find or
...
Maintain one concise checkpoint in the Zero issue or an issue document:
...
3. Build the complete reviewable OKF/planning package required by the current
   ...
   - exact service involvement and system boundaries in Zero.
4. Ground every Zero-surface claim in the maintained surface map and
...
2. Create a Zero `request_confirmation` interaction targeted at that issue
