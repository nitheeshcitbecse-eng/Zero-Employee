...
describe("worktree helpers", () => {
  ...
  it("rejects a source migration journal that diverges from the code journal", () => {
    expect(() => resolveWorktreeSeedMigrationRevision({
      ...
      journalEntryCount: 3,
    }, "sourcePrefix")).toThrow("Migration journal is not a prefix of this Zero checkout");
  });
  ...
  it("defaults the seed source config to the current repo-local Zero config", () => {
    ...
    try {
      ...
      expect(() =>
        resolveWorktreeReseedTargetPaths({
          configPath,
          rootPath: worktreeRoot,
        })).toThrow("does not look like a worktree-local Zero instance");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("repairs the current linked worktree when Zero metadata is missing", async () => {
