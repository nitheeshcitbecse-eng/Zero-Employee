...
function requiredApiResult<T>(value: T | null, action: string): T {
  if (value === null) {
    throw new Error(`Zero returned no result while ${action}.`);
  }
  return value;
}
 ...
 * a PAPERCLIP_ prefix; all other Zero routing/configuration is discarded.
 */
...
export async function reconcileTestDriveWorktreeExecution(
  ...
  if (!worktreeExecutionArmed(verified, instanceId)) {
    throw new Error(
      `Could not arm “Run tasks in this worktree” for Zero instance ${instanceId}. ` +
...
export async function testDriveCommand(
  ...
  p.log.message(pc.dim("The data directory is retained when Zero exits."));
    ...
    await dependencies.run({
      ...
      afterStart: async (server) => {
        ...
        if (options.browser === false) {
          p.log.success(`Zero is ready at ${pc.cyan(url)}.`);
          return;
        }
        const opened = await dependencies.openBrowser(url);
        if (opened) {
          p.log.success(`Zero is ready and opened at ${pc.cyan(url)}.`);
        } else {
          p.log.warn(`Zero is ready, but the browser could not be opened. Visit ${url}.`);
        }
      },
    });
  } catch (error) {
...
export function registerTestDriveCommand(program: Command): void {
  program
    .command("test-drive")
    .description("Start an isolated, initialized Zero instance for manual testing")
    .option("-d, --data-dir <path>", "Zero data directory to create or reuse")
