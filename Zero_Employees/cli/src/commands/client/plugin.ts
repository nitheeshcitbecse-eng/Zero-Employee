...
/** Result of probing the Zero instance the CLI is about to talk to. */
...
export function renderLocalPluginInstallHint(packagePath: string): string {
  return [
    ...
    pc.dim(`Keep ${pc.cyan("pnpm dev")} running in ${packagePath}; Zero watches rebuilt dist output and reloads the plugin worker.`),
  ].join("\n");
}

/**
 * Probe `GET /api/health` on the instance the CLI is configured to talk to so a
 * developer can confirm *which* Zero they are about to install into. This
...
export function formatTargetDiagnostics(diag: TargetDiagnostics): string {
  const lines = [pc.dim(`Target Zero: ${pc.cyan(diag.apiBase)}`)];
...
export function registerPluginCommands(program: Command): void {
  ...
  addCommonClientOptions(
    plugin
      ...
      .description("Scaffold a local Zero plugin project")
  ...
  addCommonClientOptions(
    plugin
      ...
      .option(
        "--no-verify-target",
        "Skip the pre-install probe that reports which Zero instance the plugin installs into",
      )
  ...
  addCommonClientOptions(
    plugin
      .command("target")
      .description(
        "Show which Zero instance plugin commands will talk to.\n" +
