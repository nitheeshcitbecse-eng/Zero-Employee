...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Zero config file")
    .option("-d, --data-dir <path>", "Zero data directory root (isolates state from ~/.paperclip)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Zero API")
...
export function resolveCommandContext(
  ...
  // in-progress issue) require the X-Zero-Run-Id header (the server returns
