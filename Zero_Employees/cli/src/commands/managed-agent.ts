...
export const CLAUDE_MANAGED_SYSTEM_PROMPT =
  "You are a Zero remote agent. Follow the current user turn and use only the custom tools supplied for that session. Zero tool authority, completion, blocking, review, and yielding are enforced by the runner. Never request or infer a Zero endpoint or credential.";
...
function resourceByProfile(
  ...
  if (matches.length > 1) {
    throw new Error(
      `Multiple Anthropic ${resourceLabel} resources use Zero profile ${profileKey}; pass an explicit resource ID`,
    );
  }
...
export function assertSafeManagedEnvironment(environment: Record<string, unknown>): void {
  ...
  if (
    ...
    throw new Error(
      "Existing Anthropic Environment does not match Zero's no-network, no-package profile",
    );
  }
}
...
async function resolveEnvironment(
  ...
  const environment = await anthropicRequest(key, "POST", "/v1/environments", {
    name: `Zero · ${options.displayName}`,
    description: "Zero remote-agent environment: no network or added packages.",
    config: {
      type: "cloud",
      networking: {
        type: "limited",
...
async function resolveAgent(
  ...
  const agent = await anthropicRequest(key, "POST", "/v1/agents", {
    name: `Zero · ${options.displayName}`,
    description: "Versioned Zero remote agent; runnerd supplies session tools.",
    model: options.model,
