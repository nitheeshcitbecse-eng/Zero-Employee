...
async function runPreUpdateBackup(options: UpdateOptions, backup: () => Promise<void>, hasInstanceData = hasPaperclipInstanceData): Promise<void> {
  if (!hasInstanceData()) {
    const message = "Skipping the pre-update backup because this Zero instance has not been onboarded and has no data to back up.";
  ...
  try {
    ...
    if (isDatabaseUnreachableError(error)) {
      throw new Error(
        "The Zero database is not running or reachable, so the pre-update backup cannot be taken. Start the service with `paperclipai service start` and retry, or skip the backup with `paperclipai update --no-backup`.",
        { cause: error },
      );
    }
    throw error;
...
export async function updateCommand(options: UpdateOptions, overrides: Partial<Dependencies> = {}): Promise<void> {
  ...
  if (mode === "source" || mode === "unknown") { emit(options, { mode, action: "manual" }, "This appears to be a source checkout. Update it with `git pull` followed by `pnpm install`; Zero will not mutate the repository."); return; }
