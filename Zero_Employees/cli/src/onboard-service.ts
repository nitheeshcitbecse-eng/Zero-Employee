...
export async function handoffToOnboardedService(
  ...
  deps.info(`Zero dashboard: ${pc.cyan(dashboardUrl)}`);
  ...
  if (await deps.openDashboard(dashboardUrl)) {
    deps.success("Sent the Zero dashboard to your browser.");
  } else {
...
const defaultDependencies: OnboardServiceDependencies = {
  ...
  confirm: async () => {
    const answer = await p.confirm({
      message: "Install Zero as a background service?",
      initialValue: true,
    });
  ...
  confirmLinger: async () => {
    const answer = await p.confirm({
      message: "Allow Zero to keep running after logout? This may request system authorization.",
      initialValue: false,
    });
    return !p.isCancel(answer) && answer === true;
  },
