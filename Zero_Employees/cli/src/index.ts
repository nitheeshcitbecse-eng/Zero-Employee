...
const DATA_DIR_OPTION_HELP =
  "Zero data directory root (isolates state from ~/.paperclip)";

program.enablePositionalOptions();

program
  .name("paperclipai")
  .description("Zero CLI — setup, diagnose, and configure your instance")
  .version(cliVersion);

program
  .command("install")
  .description("Install Zero into a managed per-user CLI store")
...
program
  .command("update")
  .alias("upgrade")
  .description("Check, update, or roll back the Zero CLI")
...
program
  ...
  .option("--run", "Start Zero immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Zero setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
const run = program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Zero")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Zero server API")
