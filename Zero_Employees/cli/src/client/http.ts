...
function buildConnectionErrorMessage(input: {
  ...
  lines.push(
    "",
    "This usually means the Zero server is not running, the configured URL is wrong, or the request is being blocked before it reaches Zero.",
    "",
    "Try:",
    "- Start Zero with `pnpm dev` (from a source checkout) or `npx paperclipai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Zero is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`PAPERCLIP_API_URL\`.`,
  );
  return lines.join("\n");
}
